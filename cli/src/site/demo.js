...
const WEBC = "webc",
  ...
  stylDemoPlugin = (svg_map) => {
    const pugRender = async (demo_dir) => {
        const pug_path = join(demo_dir, "_.pug");
        if (existsSync(pug_path)) {
          const pug_src = await read(pug_path);
          return (
            "export const HTM = " +
            JSON.stringify(pug.render(pug_src, { filename: pug_path })) +
            ";\n"
          );
        }
        return "";
          ...
          if (has_demo_i18n) {
            ...
            write(
              ...
              "export const js = " +
                ...
                ", pug = " +
                JSON.stringify(pug_val) +
                ";"
            );
          } else {
            write(out_file, compileI18n(lang_dir));
          }
        }
      }
    }
