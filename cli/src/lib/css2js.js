...
export default (css_content, css_path, js_path) => {
      ...
      if (existsSync(svg_abs_path)) {
        ...
        if (!existsSync(svg_js_path)) {
          const encoded = svgEncode(cleaned),
            svg_js_content =
              'import{css}from"../js/dom.js";css(":root{--' +
              hash +
              ':url(\\"' +
              encoded +
              '\\")}")';
  ...
  import_li.forEach((p, idx) => {
    const import_var = "i" + idx;
    import_statement_li.push("import " + import_var + " from " + JSON.stringify(p) + ";");
    call_li.push(import_var + "(root)");
  });
  ...
  const js_content =
    import_statement_li.join("") + "export default(root=D)=>{" + call_li.join(";") + "}";
