...
export default (h1, b, doc_h2, url) => {
  ...
  const titleRender = (name, doc_title, lang) => {
      ...
      edit_btn.setAttribute("href", DOC_EDIT + "/src/" + url + "/i18n/" + lang + "/README.md");
      ...
      doc_md.querySelectorAll("c-htm-demo").forEach((el) => {
        el.gen = (type, text) => {
          const css = css_li.map((x) => "@import '" + JSD + x + "_.css';").join("\n"),
            js_dep_li =
              HAS_JS && !text.includes(MOD) ? ['import "' + MOD + url + '.js";', ...js_li] : js_li;
          let htm = "",
            js = js_dep_li.join("\n");
          ...
          if (js || htm.includes("</script>")) {
            htm =
              '<script type="importmap">{"imports":{"' + MOD + '":"' + JSD + '"}}</script>\n' + htm;
          }
