...
Tan-awa ang `src/webc/BoxX/demo` ug paghimo og demo sa component sa `src/webc/component name/demo/`

`demo/_.pug`, demo subtemplate

Ang `c-t` usa ka custom nga internationalized nga tag, nga kinahanglang i-indent sa usa ka bulag nga linya isip child node (gisulat sa samang linya hugot nga gidili):
Sakto nga pananglitan:
```pug
...
Kung dili ka magsulat `@import '../_.styl'` aron ma-import ang sangkap nga ipakita, awtomatiko kini nga i-inject; gidili ang paggamit sa `@import` sa `demo/_.styl` sa pag-import sa ubang mga estilo sa sangkap. Kung ang demo nagdepende sa ubang mga sangkap (sama sa buton `.Btn`), palihug i-import ang nagsalig nga mga sangkap pinaagi sa `import "../../Btn.js"` sa `demo/_.js`.
...
Kung ang `../component name.js` (`../component name.js` mao ang magpakita sa component mismo, ug ang ubang mga dependent nga component nga gikinahanglan sa demo kinahanglan pa nga i-import), `import` lang ang gikinahanglan, ug walay espesipikong content ang ma-import, nga mahimong awtomatikong matangtang (ang framework)
