...
Tham khảo `src/webc/BoxX/demo` và phát triển bản demo của thành phần trong `src/webc/tên thành phần/demo/`

`demo/_.pug`, mẫu con demo

`c-t` là thẻ quốc tế hóa tùy chỉnh, thẻ này phải được thụt vào một dòng riêng biệt dưới dạng nút con (nghiêm cấm viết trong cùng một dòng):
Ví dụ đúng:
```pug
...
Nếu bạn không viết `@import '../_.styl'` để nhập thành phần cần trình diễn, nó sẽ tự động được chèn vào; cấm sử dụng `@import` trong `demo/_.styl` để nhập các kiểu thành phần khác. Nếu bản demo phụ thuộc vào các thành phần khác (chẳng hạn như nút `.Btn`), vui lòng nhập các thành phần phụ thuộc qua `import "../../Btn.js"` trong `demo/_.js`.
...
Nếu `../comComponent name.js` (`../comComponent name.js` để hiển thị chính thành phần đó và các thành phần phụ thuộc khác mà bản demo yêu cầu vẫn cần được nhập), thì chỉ cần nhập `import` và không có nội dung cụ thể nào được nhập, có thể bỏ qua nội dung này (khung sẽ tự động chèn nội dung đó)
