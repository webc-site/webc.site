  ...
  Đối với thành phần `js`, hãy triển khai thành phần bằng cách sử dụng `Light DOM` trong `src/webc/comComponentname.js`.

  Sử dụng đường dẫn tương đối để nhập các thành phần khác, chẳng hạn như: `import "./comComponent name.js"` (Trong tệp JS của từng thành phần, trước tiên hãy nhập các phần phụ thuộc, sau đó nhập `_.styl` tương ứng, ví dụ `import "./comComponent name/_.styl";`).
  ...
  Khi lệnh gọi lại đang tải, hãy sử dụng `const xx = newEl('b');xx.className="Ing"` để hiển thị hoạt ảnh và giới thiệu các phần phụ thuộc thông qua `import "./Ing.js"` trong tệp JS thành phần.