...
參考 `src/webc/BoxX/demo` ，在 `src/webc/組件名/demo/` 開發組件的演示

`demo/_.pug`，示範子模板

`c-t` 為自訂國際化標籤，必須單獨縮排成一行作為子節點（嚴禁寫在同行）：
正確範例：
```pug
...
不寫 `@import '../_.styl'` 導入待演示元件本身，會自動注入；禁止在 `demo/_.styl` 中使用 `@import` 導入其他元件樣式。如 demo 依賴其他元件（如按鈕 `.Btn`），請在 `demo/_.js` 中透過 `import "../../Btn.js"` 匯入依賴元件。
...
若 `../元件名稱.js` (`../元件名稱.js` 是展示元件本身 ，其他demo 所需的依賴元件還是要匯入) 只要 `import` ，不匯入特定內容，可省略（框架會自動注入）
