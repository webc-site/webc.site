...
Lihat `src/webc/BoxX/demo` dan kembangkan demo komponen di `src/webc/component name/demo/`

`demo/_.pug`, subtemplat demo

`c-t` adalah tag khusus yang diinternasionalkan, yang harus diindentasi ke dalam baris terpisah sebagai simpul anak (dilarang keras menulis di baris yang sama):
Contoh yang benar:
```pug
...
Jika Anda tidak menulis `@import '../_.styl'` untuk mengimpor komponen yang akan didemonstrasikan, komponen tersebut akan disuntikkan secara otomatis; dilarang menggunakan `@import` di `demo/_.styl` untuk mengimpor gaya komponen lainnya. Jika demo bergantung pada komponen lain (seperti tombol `.Btn`), harap impor komponen bergantung melalui `import "../../Btn.js"` di `demo/_.js`.
...
Jika `../component name.js` (`../component name.js` adalah untuk menampilkan komponen itu sendiri, dan komponen dependen lainnya yang diperlukan oleh demo masih perlu diimpor), hanya `import` yang diperlukan, dan tidak ada konten spesifik yang diimpor, yang dapat dihilangkan (kerangka kerja akan secara otomatis memasukkannya)
