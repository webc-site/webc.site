...
`src/webc/BoxX/demo` ga qarang va komponentning demo versiyasini `src/webc/component name/demo/` da ishlab chiqing.

`demo/_.pug`, demo shablon

`c-t` - bu maxsus xalqaro yorliq bo'lib, u alohida qatorga ichki tugun sifatida kiritilishi kerak (bir qatorga yozish qat'iyan man etiladi):
To'g'ri misol:
```pug
...
Agar siz ko'rsatiladigan komponentni import qilish uchun `@import '../_.styl'` yozmasangiz, u avtomatik ravishda kiritiladi; boshqa komponent uslublarini import qilish uchun `demo/_.styl` da `@import` dan foydalanish taqiqlanadi. Agar demo boshqa komponentlarga (masalan, tugma `.Btn`) bog'liq bo'lsa, iltimos, `demo/_.js` ichidagi `import "../../Btn.js"` orqali bog'liq komponentlarni import qiling.
...
Agar `../component name.js` (`../component name.js` komponentning o'zini ko'rsatishi kerak bo'lsa va demo tomonidan talab qilinadigan boshqa qaram komponentlar hali ham import qilinishi kerak bo'lsa), faqat `import` kerak bo'ladi va hech qanday maxsus tarkib import qilinmaydi, bu esa avtomatik ravishda o'tkazib yuborilishi mumkin (ramkada)
