...
Reportez-vous à `src/webc/BoxX/demo` et développez une démo du composant dans `src/webc/component name/demo/`

`demo/_.pug`, sous-modèle de démonstration

`c-t` est une balise internationalisée personnalisée, qui doit être indentée sur une ligne distincte en tant que nœud enfant (l'écriture sur la même ligne est strictement interdite) :
Exemple correct :
```pug
...
Si vous n'écrivez pas `@import '../_.styl'` pour importer le composant à démontrer, il sera automatiquement injecté ; il est interdit d'utiliser `@import` dans `demo/_.styl` pour importer d'autres styles de composants. Si la démo dépend d'autres composants (tels que le bouton `.Btn`), veuillez importer les composants dépendants via `import "../../Btn.js"` dans `demo/_.js`.
...
Si `../component name.js` (`../component name.js` doit afficher le composant lui-même et que les autres composants dépendants requis par la démo doivent encore être importés), seul `import` est requis et aucun contenu spécifique n'est importé, qui peut être omis (le framework l'injectera automatiquement)
