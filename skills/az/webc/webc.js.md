  ...
  `js` komponenti üçün komponenti `src/component name/_.js`-də həyata keçirmək üçün `Light DOM` istifadə edin.

  Digər komponentləri idxal etmək üçün nisbi yollardan istifadə edin, məsələn: `import "../component name/_.js"` (`js` idxal etmir `styl`, çərçivə avtomatik olaraq onu yeridəcək).
  ...
  Geri zəng yüklənərkən, `_.styl`-də `const xx = newEl('b');xx.className="Ing"` və `@import "../Ing/_.styl"` ilə animasiya göstərin