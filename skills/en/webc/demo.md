...
Refer to `src/BoxX/demo` and develop a demo of the component in `src/component name/demo/`
...
Do not write `@import '../_.styl'` to import the component to be demonstrated, it will be automatically injected

Please import `@import '../../Btn/_.styl'` in `demo/_.styl` and use `.Btn`
...
If `../_.js` (`../_.js` is to display the component itself, the dependent components required by other demos still need to be imported) only `import` is required, and no specific content is imported, which can be omitted (the framework will automatically inject it)
