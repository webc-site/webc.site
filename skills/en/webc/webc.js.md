  ...
  For the `js` component, use `Light DOM` to implement the component in `src/component name/_.js`.

  Use relative paths to import other components, such as: `import "../component name/_.js"` (`js` does not import `styl`, the framework will automatically inject it).
  ...
  While the callback is loading, show animation with `const xx = newEl('b');xx.className="Ing"` and `@import "../Ing/_.styl"` in `_.styl`