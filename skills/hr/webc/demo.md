...
Pogledajte `src/webc/BoxX/demo` i razvijte demonstraciju komponente u `src/webc/component name/demo/`

`demo/_.pug`, demo podpredložak

`c-t` je prilagođena internacionalizirana oznaka, koja mora biti uvučena u zaseban redak kao podređeni čvor (pisanje u istom retku strogo je zabranjeno):
Točan primjer:
```pug
...
Ako ne napišete `@import '../_.styl'` za uvoz komponente koja se demonstrira, ona će se automatski ubaciti; zabranjeno je koristiti `@import` u `demo/_.styl` za uvoz drugih stilova komponenti. Ako demo ovisi o drugim komponentama (kao što je gumb `.Btn`), uvezite zavisne komponente kroz `import "../../Btn.js"` u `demo/_.js`.
...
Ako `../component name.js` (`../component name.js` treba prikazati samu komponentu, a druge ovisne komponente koje zahtijeva demonstracija i dalje treba uvesti), potreban je samo `import` i ne uvozi se određeni sadržaj koji se može izostaviti (okvir će ga automatski ubaciti)
