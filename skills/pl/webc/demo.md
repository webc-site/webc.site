...
Zapoznaj się z `src/BoxX/demo` i opracuj wersję demonstracyjną komponentu w `src/component name/demo/`
...
Nie pisz `@import '../_.styl'`, aby zaimportować komponent, który chcesz zademonstrować, zostanie on automatycznie wstrzyknięty

Proszę zaimportować `@import '../../Btn/_.styl'` do `demo/_.styl` i użyć `.Btn`
...
Jeśli `../_.js` (`../_.js` ma wyświetlić sam komponent, zależne komponenty wymagane przez inne dema nadal muszą zostać zaimportowane) wymagane jest tylko `import` i nie jest importowana żadna konkretna treść, którą można pominąć (framework automatycznie ją wstrzyknie)
