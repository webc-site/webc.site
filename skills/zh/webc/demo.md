...
参考 `src/BoxX/demo` ，在 `src/组件名/demo/` 开发组件的演示
...
不写 `@import '../_.styl'` 导入待演示组件本身，会自动注入

按钮请在 `demo/_.styl` 导入 `@import '../../Btn/_.styl'`，用 `.Btn`
...
若 `../_.js` (`../_.js` 是展示组件自身 ，其他demo 需要的依赖组件还是要导入) 仅需 `import` ，不导入具体内容，可省略（框架会自动注入）
